...
pub mod pallet {
	...
	#[derive(
		Clone, Encode, Decode, Default, Eq, MaxEncodedLen, PartialEq, RuntimeDebug, TypeInfo,
	)]
	...
	pub struct RoundDetails<Balance, BlockNumber, BoundedSubmissions> {
		pub submissions: BoundedSubmissions,
		pub submission_count_bounds: (u32, u32),
		pub payment: Balance,
		pub timeout: BlockNumber,
	}
	...
	pub type RoundDetailsOf<T> = RoundDetails<
		BalanceOf<T>,
		<T as frame_system::Config>::BlockNumber,
		BoundedVec<<T as Config>::Value, <T as Config>::SubmissionLimit>,
	>;
		...
		Value: Default,
	{
		...
		fn new(starting_round: RoundId) -> Self {
			Self { starting_round, ..Default::default() }
		}
	}
	...
	pub trait Config: frame_system::Config {
		...
		/// The maximum number of submissions (per round).
		#[pallet::constant]
		type SubmissionLimit: Get<u32>;
	}
	...
	pub enum Event<T: Config> {
		...
		RoundDetailsUpdated(T::FeedId, BalanceOf<T>, SubmissionBounds, u32, T::BlockNumber),
	...
	pub enum Error<T> {
		...
		MaxDebtReached,
		/// The maximum number of submissions was reached.
	...
	impl<T: Config> Pallet<T> {
		...
		/// - `timeout`: The timeout in blocks, after which a started round is timed out. If a round
		///   started in block `n` and the feed's `timeout` is `10`, then the round is timed out if
		///   `n + 10 < current_block`.
		/// - `submission_value_bounds`: The value bounds of oracle submissions that are considered
		///   valid.
		/// - `min_submissions`: Determines the lower end of the feed's `submission_count_bounds`
		///   where the maximum number of submissions a round records is the number of configured
		///   oracles and the lower end is the minimum amount of submissions a round needs to update
		///   an answer via `AnswerUpdated` event.
		/// - `decimals`: Metadata that represents the number of decimals with which the feed is
		///   configured.
		/// - `description`: A user friendly name or ticker of this feed. Limited in length by
		///   `StringLimit`.
		/// - `restart_delay`: The number of rounds that must elapse before a new round can be
		///   initiated by the same oracle again. Therefore this must be lower than the number of
		///   provided oracles.
		/// - `oracles`: A list of the oracle accounts together with their admin account (`oracle`,
		///   `admin`) that will be registered as oracles for the feed. If the oracle account is
		///   already tracked (for another feed for example), then the provided admin must match the
		///   already tracked oracle's admin (see `Oracles`). At least one (`oracle`, `admin`) pair
		///   is required.
		/// - `pruning_window`: This specifies the number of rounds to keep in storage for this
		///   feed. A  `None` ensures that no past rounds are ever deleted from the chain's storage.
		///   A value of `Some(n)` ensures that `n` rounds are kept in storage after the total
		///   number of rounds has exceeded `n`. This means that starting with the `n+1` round, the
		///   oldest round is purged from storage.
		/// - `max_debt`: With `Some(n)` the feed is allowed to accumulate a less or equal than `n`
		///   amount of debt to the oracles. This means that oracles can submit values even if the
		///   feed's fund is dry, and essentially receive an IOU that they can cash in once the feed
		///   is refunded.
		///
		...
		pub fn cancel_ownership_transfer(
			...
			if let Some(pending_owner) = feed.config.pending_owner.take() {
				Self::deposit_event(Event::OwnerUpdateCanceled(feed_id, old_owner, pending_owner));
				Ok(())
			} else {
		...
		///
		/// - Will start a new round if there is no round for the id, yet, and a round can be
		///   started (at this time by this oracle).
		...
		pub fn submit(
			...
			let new_round_id =
				feed.reporting_round_id().checked_add(One::one()).ok_or(Error::<T>::Overflow)?;
			...
			details
				.submissions
				.try_push(submission)
				.map_err(|_| Error::<T>::MaxSubmissionsReached)?;
			...
			oracle_meta.withdrawable =
				oracle_meta.withdrawable.checked_add(&payment).ok_or(Error::<T>::Overflow)?;
		...
		pub fn request_new_round(
			...
			let next_allowed_round =
				last_started.checked_add(requester.delay).ok_or(Error::<T>::Overflow)?;
			ensure!(
		...
		pub fn withdraw_funds(
			...
			let sender = ensure_signed(origin)?;
			ensure!(Some(sender) == Self::pallet_admin(), Error::<T>::NotPalletAdmin);
			let fund = Self::account_id();
		...
		pub fn transfer_pallet_admin(
			...
			ensure!(Self::pallet_admin().as_ref() == Some(&old_admin), Error::<T>::NotPalletAdmin);
		...
		pub fn accept_pallet_admin(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
			let new_pallet_admin = ensure_signed(origin)?;
				...
				PendingPalletAdmin::<T>::get().filter(|p| p == &new_pallet_admin).is_some(),
		...
		pub fn set_feed_creator(
			...
			let admin = ensure_signed(origin)?;
			ensure!(Self::pallet_admin() == Some(admin), Error::<T>::NotPalletAdmin);
	...
	/// `should_sync` flag determines whether the `config` is put into storage on `drop`.
	...
	impl<T: Config> Feed<T> {
		...
		pub fn new(id: T::FeedId, config: FeedConfigOf<T>) -> Self {
			Self { id, config, should_sync: true }
		}
		...
		pub fn read_only_from(id: T::FeedId) -> Option<Self> {
			...
			Some(Self { id, config, should_sync: false })
		}
		...
		pub fn load_from(id: T::FeedId) -> Option<Self> {
			let config = Feeds::<T>::get(id)?;
			Some(Self { id, config, should_sync: true })
		}
		...
		fn is_timed_out(&self, round: RoundId) -> bool {
			...
			let started_at = self.round(round).map(|r| r.started_at).unwrap_or_else(Zero::zero);
			let timeout = self.details(round).map(|d| d.timeout).unwrap_or_else(Zero::zero);
		...
		fn do_add_oracles(&mut self, to_add: Vec<(T::AccountId, T::AccountId)>) -> DispatchResult {
			...
			for (oracle, admin) in to_add {
				if let Some(meta) = Oracles::<T>::get(&oracle) {
					...
					Oracles::<T>::insert(
						&oracle,
						OracleMeta { withdrawable: Zero::zero(), admin, pending_admin: None },
					);
				}
				OracleStatuses::<T>::try_mutate(
					...
					|maybe_status| -> DispatchResult {
						...
						// in order to keep the count accurate.
							...
							maybe_status.as_ref().map(|s| s.ending_round.is_some()).unwrap_or(true),
		...
		fn do_update_future_rounds(
			...
			// round.
			ensure!(self.oracle_count() > restart_delay, Error::<T>::DelayNotBelowCount);
		...
		fn current_window(&self) -> u32 {
			self.config.latest_round.saturating_sub(self.config.next_round_to_prune)
		}
		...
		fn initialize_round(
			...
			Details::<T>::insert(
				...
				new_round_id,
				RoundDetails {
					submissions: BoundedVec::default(),
	...
	impl<T: Config> MutableFeedInterface<T> for Feed<T> {
		...
		fn request_new_round(&mut self, requester: T::AccountId) -> DispatchResult {
			let new_round =
				self.reporting_round_id().checked_add(One::one()).ok_or(Error::<T>::Overflow)?;
			ensure!(
