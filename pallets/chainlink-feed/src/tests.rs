...
use frame_support::{
	...
	storage::{with_transaction, TransactionOutcome, TransactionOutcome::*},
...
fn feed_creation_failure_cases() {
	new_test_ext().execute_with(|| {
		...
		let too_many_oracles = (0..(OracleLimit::get() + 1))
			...
			.collect();
		assert_noop!(
			FeedBuilder::new().oracles(too_many_oracles).build_and_store(),
			Error::<Test>::OraclesLimitExceeded
		);
		assert_noop!(
			FeedBuilder::new().min_submissions(3).oracles(vec![(1, 2)]).build_and_store(),
			Error::<Test>::WrongBounds
		);
		assert_noop!(
			FeedBuilder::new().min_submissions(0).oracles(vec![(1, 2)]).build_and_store(),
			Error::<Test>::WrongBounds
		);
		assert_noop!(
			FeedBuilder::new()
...
fn submit_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			details,
			RoundDetails {
				submissions: bounded_vec![submission, submission],
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			System::events()
				...
				.map(|r| r.event)
				.filter_map(|e| {
					if let mock::RuntimeEvent::ChainlinkFeed(Event::NewData(feed, data)) = e {
						Some((feed, data))
					} else {
						None
					}
				...
				.last()
				.unwrap(),
			(0, RoundData { started_at: 1, answer: 42, updated_at: 1, answered_in_round: 1 })
		);
	});
}
...
fn submit_failure_cases() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(FeedBuilder::new().value_bounds(1, 100).oracles(oracles).build_and_store());
		...
		let not_oracle = 1337;
		assert_noop!(
			ChainlinkFeed::submit(RuntimeOrigin::signed(not_oracle), feed_id, round_id, submission),
...
fn change_oracles_should_work() {
	new_test_ext().execute_with(|| {
		...
		let to_disable: Vec<u64> =
			initial_oracles.iter().cloned().take(2).map(|(o, _a)| o).collect();
		...
		for o in to_disable.iter() {
			assert!(
				ChainlinkFeed::oracle_status(feed_id, o).unwrap().ending_round.is_some(),
		...
		assert_eq!(ChainlinkFeed::oracle_status(feed_id, oracle), Some(expected_status));
	});
}
...
fn force_admin_transfer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ChainlinkFeed::force_set_pallet_admin(RuntimeOrigin::root(), new_admin));
...
fn admin_transfer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ChainlinkFeed::cancel_admin_transfer(RuntimeOrigin::signed(old_admin), oracle,),);
		assert_noop!(
		...
		assert_ok!(ChainlinkFeed::accept_admin(RuntimeOrigin::signed(new_admin), oracle));
		let oracle_meta = ChainlinkFeed::oracle(oracle).expect("oracle should be present");
...
fn request_new_round_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(requester_meta, Requester { delay, last_started_round: None });
		// failure cases
		assert_noop!(
		...
		assert_ok!(ChainlinkFeed::request_new_round(RuntimeOrigin::signed(requester), feed_id));
		...
		let round_id = 1;
		let round = ChainlinkFeed::round(feed_id, round_id).expect("first round should be present");
		assert_eq!(round, Round { started_at: 0, ..Default::default() });
		let details = ChainlinkFeed::round_details(feed_id, round_id)
			.expect("details for first round should be present");
		assert_eq!(
			details,
			RoundDetails { submissions: bounded_vec![], submission_count_bounds, payment, timeout }
		);
		let requester_meta =
			ChainlinkFeed::requester(feed_id, requester).expect("requester should be present");
		assert_eq!(requester_meta, Requester { delay, last_started_round: Some(1) });
	});
}
...
fn transfer_ownership_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ChainlinkFeed::accept_ownership(RuntimeOrigin::signed(new_owner), feed_id));
		assert_noop!(
			ChainlinkFeed::cancel_ownership_transfer(RuntimeOrigin::signed(old_owner), feed_id,),
			Error::<Test>::NotFeedOwner
		);
...
fn feed_oracle_trait_should_work() {
	new_test_ext().execute_with(|| {
		...
		));
		{
			...
			assert_eq!(
				feed.latest_data(),
				RoundData { answer: 42, started_at: 0, updated_at: 0, answered_in_round: 1 }
			);
...
fn payment_withdrawal_should_work() {
	new_test_ext().execute_with(|| {
		...
		Oracles::<Test>::insert(
			oracle,
			OracleMeta { withdrawable: amount, admin, ..Default::default() },
		);
		assert_noop!(
...
fn funds_withdrawal_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ChainlinkFeed::withdraw_funds(RuntimeOrigin::signed(fund), recipient, amount));
	});
}
...
fn transfer_pallet_admin_should_work() {
	new_test_ext().execute_with(|| {
		...
		// transfer to self yields nothing
		assert_ok!(ChainlinkFeed::transfer_pallet_admin(RuntimeOrigin::signed(fund), fund));
		assert!(System::events().into_iter().all(|r| {
			!matches!(
				r.event,
		...
		assert_ok!(ChainlinkFeed::transfer_pallet_admin(RuntimeOrigin::signed(fund), new_admin));
		...
		assert_ok!(ChainlinkFeed::cancel_pallet_admin_transfer(RuntimeOrigin::signed(fund)),);
		...
		assert_ok!(ChainlinkFeed::accept_pallet_admin(RuntimeOrigin::signed(new_admin)));
...
fn auto_prune_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ChainlinkFeed::set_pruning_window(RuntimeOrigin::signed(owner), feed_id, 1));
		let feed = ChainlinkFeed::feed(feed_id).expect("feed should be there");
...
fn feed_creation_permissioning() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ChainlinkFeed::set_feed_creator(RuntimeOrigin::signed(admin), new_creator));
		...
		assert_ok!(ChainlinkFeed::remove_feed_creator(RuntimeOrigin::signed(admin), new_creator));
		assert_noop!(
			FeedBuilder::new().owner(new_creator).build_and_store(),
			Error::<Test>::NotFeedCreator
		);
...
fn can_go_into_debt_and_repay() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ChainlinkFeed::submit(RuntimeOrigin::signed(oracle), 0, 1, 42));
		...
		assert_ok!(ChainlinkFeed::reduce_debt(RuntimeOrigin::signed(admin), 0, 10));
		...
		assert_ok!(ChainlinkFeed::reduce_debt(RuntimeOrigin::signed(admin), 0, payment));
...
fn feed_life_cycle() {
	new_test_ext().execute_with(|| {
		...
		let new_config = FeedConfig { oracle_count: oracles.len() as u32, ..new_config };
		...
		let modified_config = FeedConfig { timeout: new_timeout, ..new_config };
...
fn allows_submissions_until_max_debt() {
	new_test_ext().execute_with(|| {
		...
		let oracles =
			vec![(1, oracle_admin), (2, oracle_admin), (3, oracle_admin), (5, oracle_admin)];
		// create two feeds
		assert_ok!(FeedBuilder::new()
			.payment(payment)
			.owner(owner)
		...
		assert_ok!(ChainlinkFeed::reduce_debt(RuntimeOrigin::signed(admin), 0, max_debt));
