use frame_support::{
	...
	sp_runtime::traits::AccountIdConversion, traits::Everything, PalletId,
};
...
parameter_types! {
	...
	pub const SubmissionLimit: u32 = 10;
}

type FeedId = u16;
type Value = u64;

impl traits::OnAnswerHandler<Test> for Test {
	fn on_answer(feed: FeedId, new_data: RoundData<BlockNumber, Value>) {
		ChainlinkFeed::deposit_event(Event::NewData(feed, new_data));
	}
}
...
impl Config for Test {
	...
	type SubmissionLimit = ConstU32<10>;
}
...
pub fn new_test_ext_with_feeds(feeds: Vec<FeedBuilderOf<Test>>) -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Test> { balances: vec![(pallet_account, 100 * MIN_RESERVE)] }
